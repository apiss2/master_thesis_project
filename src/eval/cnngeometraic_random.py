# 汎用ライブラリ
import os
import cv2
import argparse
import numpy as np
import pandas as pd
from tqdm import tqdm
import segmentation_models_pytorch as smp
# torch関連
import torch
import torch.nn as nn
from torch.utils.data import Dataset, DataLoader
from torchvision import transforms
# 自作ファイル
from custom_dataset import Dataset_fromDir, PairImageDataset
from models import CNNGeometric
from transform_util import CenterTransform

if __name__ == '__main__':
    #各種定数の設定
    use_cuda = torch.cuda.is_available()
    device = torch.device('cuda') if use_cuda else torch.device('cpu')
    base = args.base
    size = args.size

    print('-------------------------------------')
    print('モデルのロード中...')
    ch = args.fe_output_channel
    s = args.fe_output_size
    output_dim_1 = geometric_choice(args.geometric_1)
    model_1 = CNNGeometric(select_model=args.select_model,
                output_dim=output_dim_1, fe_output_shape=(ch,s,s),
                fr_channels=[ch,128,64], use_cuda=use_cuda,
                pretrain_path=args.FE_model)
    model_1.FeatureRegression.load_state_dict(torch.load(args.model_1))
    model_1.eval()
    if args.two_stage:
        output_dim_2 = geometric_choice(args.geometric_2)
        model_2 = CNNGeometric(select_model=args.select_model,
                    output_dim=output_dim_2, fe_output_shape=(ch,s,s),
                    fr_channels=[ch,128,64], use_cuda=use_cuda,
                    pretrain_path=args.FE_model)
        model_2.FeatureRegression.load_state_dict(torch.load(args.model_2))
        model_2.eval()

    print('-------------------------------------')
    print('dataloaderの作成中...')
    train_tfms = transforms.Compose([transforms.RandomRotation(15),
                                    transforms.RandomAffine(10, \
                                        translate=(0.05,0.05), scale=(0.9,1.1), shear=0.05),
                                    transforms.RandomVerticalFlip(p=0.5)
                                    ])
    dataset = Dataset_fromDir(dataset_image_path=args.src_image_path,
                        dataset_label_path=args.src_label_path,
                        center_crop=base,
                        transform=train_tfms,
                        geometric=args.geometric_1)
    dataloader = DataLoader(dataset, batch_size=1,
                            shuffle=False, num_workers=0)
    center_transform = CenterTransform(use_cuda=use_cuda, geometric=args.geometric_1,\
                        base=base, size=size)
    criterion = smp.utils.losses.DiceLoss()
    loss=0

    print('-------------------------------------')
    print('評価中...')
    with torch.no_grad():
        for i, batch in tqdm(enumerate(dataloader)):
            # gpu(cpu)に移動
            src_image = batch['image'].to(device)
            src_label = batch['label'].to(device)
            theta = batch['theta'].to(device)
            # 画像をランダムに変形させて疑似ラベルを作成(size×size)
            tgt_image = center_transform(src_image, theta)
            tgt_label = center_transform(src_label, theta)
            # 元画像(base×base)の中心を切り抜く(->size×size)
            src_image = center_transform.center_crop(src_image)
            src_label = center_transform.center_crop(src_label)

            # モデルの予測
            pred_theta = model_1(src_image, tgt_image)
            # 予測によって画像を変形
            pred_label = center_transform(src_label, pred_theta)
            # Dice係数を計算
            loss += criterion(tgt_label, pred_label).item()

            if args.save:
                pass ######将来課題

    print('-------------------------------------')
    print("Dice: %5f\n"%(1-loss/len(dataloader)))